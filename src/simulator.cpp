...
//    dims[0] = static_cast<unsigned int>(xres);  // w
//    dims[1] = static_cast<unsigned int>(yres);  // h
//    continuousInc[0] = 0;
//    continuousInc[1] = 0;
//    continuousInc[2] = 0;
...
//        this->PBO_Zbuffer_B->Bind(vtkPixelBufferObject::PACKED_BUFFER);
//        glReadPixels(0, 0, xres, yres, GL_DEPTH_COMPONENT, GL_FLOAT,
//                     static_cast<GLfloat *>(NULL));


//        // PBO to client
//        this->PBO_Zbuffer_A->Bind(vtkPixelBufferObject::PACKED_BUFFER);
//        if (!this->PBO_Zbuffer_A->Download2D(VTK_FLOAT, this->RawZBuffer, dims, 1, continuousInc))
//            return false;
...
//        // PBO to client
//        this->PBO_Zbuffer_B->Bind(vtkPixelBufferObject::PACKED_BUFFER);
//        if (!this->PBO_Zbuffer_B->Download2D(VTK_FLOAT, this->RawZBuffer, dims, 1, continuousInc))
//            return false;
//    }
...
//    comp_proj_transform_cv = comp_proj_transform_cv.inv();
//    cv::Mat camera2world_transform = view_transform_cv * comp_proj_transform_cv;
//    cv::Mat camera_coords = cv::Mat::ones(4, xres * yres, CV_32FC1);
//    for (int y = 0; y < yres; ++y)
//    {
//        for (int x = 0; x < xres; ++x)
//        {
//            int index = y * xres + x;
//            camera_coords.at<float>(0, index) = dwidth  * float (x) - 1.0f;
//            camera_coords.at<float>(1, index) = dheight * float (y) - 1.0f;
//            camera_coords.at<float>(2, index) = depth[index];
//        }
//    }
...
//        {
//            // NB: Convert to OpenCV coordinate, VTK's origin is at bottom-left
//            cv::Vec3f &pt = cloud.at<cv::Vec3f>(yres - (y + 1), x);

//            if (depth[ptr] == 1.0)
//            {
//                pt[0] = pt[1] = pt[2] = std::numeric_limits<float>::quiet_NaN ();
//                continue;
//            }

//            float w3 = 1.0f / world_coords.at<float>(3, ptr);

//            // vtk view coordinate system is different than the standard camera coordinates (z forward, y down, x right), thus, the fliping in y and z
//            pt[0] = world_coords.at<float>(0, ptr) * w3;
//            pt[1] = - world_coords.at<float>(1, ptr) * w3;
//            pt[2] = - world_coords.at<float>(2, ptr) * w3;
//        }
//    }
